...
llvm::Value* hossa::SubWI64::to_llvm(std::unordered_map<const hossa::Param*, llvm::Value*> env, llvm::LLVMContext& llvm_ctx, llvm::IRBuilder<>& builder) const {
    auto l = args[0]->to_llvm(env, llvm_ctx, builder);
    auto r = args[1]->to_llvm(env, llvm_ctx, builder);
    return builder.CreateSub(l, r);
}

llvm::Value* hossa::MulWI64::to_llvm(std::unordered_map<const hossa::Param*, llvm::Value*> env, llvm::LLVMContext& llvm_ctx, llvm::IRBuilder<>& builder) const {
    auto l = args[0]->to_llvm(env, llvm_ctx, builder);
    auto r = args[1]->to_llvm(env, llvm_ctx, builder);
